function [ID,timeArray,Lux,CLA,CS,Activity,Rlux,Glux,Blux,cal] = CalibrateDimesimeterDownloadFile_21Feb2013(fileName)
...
int = 10*60; % logging interval in seconds (10 minutes)
...
for i = 24:8:length(raw) % first 24 bytes are header information
    ...
    timeArray(i/8 - 2) = datenum(start) + (int/86400)*(i/8 - 3);
...
for i = 1:length(red)
    ...
    if((red(i) > 10000) && (green(i) == 0) && (blue(i) > 10000))
        ...
        %delete reading on odd # of resets
            ...
            timeArray(i) = [];
            for count = i:length(timeArray)
                timeArray(count) = timeArray(count) - (timeArray(3) - timeArray(2));
            end
            red(i) = [];
            green(i) = [];
            blue(i) = [];
...
if(resets > 0)
    display('Reset times:');
    display(datestr(timeArray(reset)));
end
