function pseudoMillerPlot(time,activity,lux,plotTitle,lightsOn,lightsOff,buffer)
...
lux(end-extra:end) = [];
activity(end-extra:end) = [];
lux = reshape(lux,dayIdx,[]);
activity = reshape(activity,dayIdx,[]);
...
delta = -find(time >= ceil(time(1)),1,'first');
...
% Create plots
[haxes,hline1,hline2] = plotyy(hour,mAI,hour,mLux,'area','semilogy');
set(hline2,...
    'Color',[0.6 0.6 0.6],...
